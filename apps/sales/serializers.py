...
class VentaSerializer(serializers.ModelSerializer):
    ...
    nombre_producto = serializers.CharField(
        source="id_producto.nombre_plan", read_only=True
    )
    ...
    def create(self, validated_data):
        ...
        permiso_sede = (
            PermisoAcceso.objects.filter(
                id_usuario=user, id_modalidad_sede__activo=True
            )
            .select_related("id_modalidad_sede")
            .first()
        )
        ...
        if not supervisor_activo:
            raise serializers.ValidationError(
                {"error": "La sede no tiene supervisor activo hoy."}
            )
    ...
    def update(self, instance, validated_data):
        ...
        # =======================================================
        if (
            instance.id_estado_sot
            and instance.id_estado_sot.codigo.upper() == "RECHAZADO"
        ):
            raise serializers.ValidationError(
                {
        ...
        with transaction.atomic():
            ...
            if nuevo_estado_audio and nuevo_estado_audio != instance.id_estado_audios:
                ...
                if nuevo_estado_audio.codigo.upper() == "RECHAZADO":
                    ...
                    estado_rechazado = EstadoSOT.objects.filter(
                        codigo__iexact="RECHAZADO"
                    ).first()
                    if estado_rechazado:
                    ...
                    if (
                        not validated_data.get("fecha_rechazo")
                        and not instance.fecha_rechazo
                    ):
                        raise serializers.ValidationError(
                            {
            ...
            if "id_estado_sot" not in validated_data:
                ...
                if nueva_fecha_inst:
                    estado_atendido = EstadoSOT.objects.filter(
                        codigo__iexact="ATENDIDO"
                    ).first()
                    if estado_atendido:
                        validated_data["id_estado_sot"] = estado_atendido

                # B. Si ingresan códigos SEC/SOT -> Sugerir EJECUCION
                elif (nuevo_codigo_sot and not instance.codigo_sot) or (
                    nuevo_codigo_sec and not instance.codigo_sec
                ):
                    estado_ejecucion = EstadoSOT.objects.filter(
                        codigo__iexact="EJECUCION"
                    ).first()
                    if estado_ejecucion:
            ...
            # =======================================================
            estado_destino = (
                validated_data["id_estado_sot"]
                if "id_estado_sot" in validated_data
                else instance.id_estado_sot
            )
            estado_audio_destino = (
                validated_data["id_estado_audios"]
                if "id_estado_audios" in validated_data
                else instance.id_estado_audios
            )
            ...
            if estado_destino and estado_destino.codigo.upper() == "ATENDIDO":
                if (
                    not estado_audio_destino
                    or estado_audio_destino.codigo.upper() != "CONFORME"
                ):
                    raise serializers.ValidationError(
                        {
                ...
                for audio_item in audios_data:
                    ...
                    if audio_id and audio_id in audios_existentes:
                        ...
                        if nueva_url and nueva_url != url_anterior:
                            ...
                            audio_instance.corregido = (
                                True  # ¡BANDERA ARRIBA! Avisamos a Jadira
                            )

                        # -----------------------------------------------------------
                        # A2. ACTUALIZACIÓN DE ETIQUETA (SI CAMBIÓ)
                        # -----------------------------------------------------------
                        audio_instance.nombre_etiqueta = audio_item.get(
                            "nombre_etiqueta", audio_instance.nombre_etiqueta
                        )
